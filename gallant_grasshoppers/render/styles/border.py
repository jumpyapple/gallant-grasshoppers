...
def draw(terminal: Terminal, window: object) -> str:
    ...
    for i in range(window.height):
        ...
        border += (
            terminal.move_xy(window.begin_x + window.width - 1, window.begin_y + i)
            + "│"
        )
    border += terminal.move_xy(window.begin_x, window.begin_y) + "─" * window.width
    border += (
        terminal.move_xy(window.begin_x, window.begin_y + window.height - 1)
        + "─" * window.width
    )
    ...
    border += (
        terminal.move_xy(
            window.begin_x + window.width - 1, window.begin_y + window.height - 1
        )
        + "┘"
    )
